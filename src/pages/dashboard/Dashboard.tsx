...
const Dashboard = () => {
  ...
  const handleSimulate = async () => {
    ...
    try {
      ...
      setError('Falha crítica de comunicação com o motor de simulação. Verifique a API.');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="flex flex-col gap-6 font-sans">
      ...
      <div className="border-b-2 border-[#003366] pb-2 flex justify-between items-center">
        ...
        <span className="text-slate-500 font-mono text-xs p-2 bg-slate-100 rounded">Status: <span className="text-[#198754] font-bold">Operacional</span></span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-[#F8FAFC] border border-slate-200 p-6 rounded-lg shadow-[inset_0_1px_3px_rgba(0,0,0,0.02)] flex items-center gap-6">
          ...
          <div className="flex flex-col">
            <span className="text-slate-500 text-sm font-semibold uppercase tracking-wider">Produtos Cadastrados</span>
        ...
        <div className="bg-[#F8FAFC] border border-slate-200 p-6 rounded-lg shadow-[inset_0_1px_3px_rgba(0,0,0,0.02)] flex items-center gap-6">
          ...
          <div className="flex flex-col">
            <span className="text-slate-500 text-sm font-semibold uppercase tracking-wider">Insumos em Estoque</span>
        ...
        <div className="bg-[#e9f5ec] border-2 border-[#198754] border-dashed p-6 rounded-lg flex items-center gap-6">
          ...
          <div className="flex flex-col">
            <span className="text-[#198754] text-sm font-semibold uppercase tracking-wider">Lucro Potencial Projetado</span>
      ...
      <div className="bg-[#F1F5F9] border border-slate-300 p-8 rounded-lg shadow-inner flex flex-col md:flex-row items-center justify-between gap-6">
        <div>
          <h2 className="text-xl font-extrabold text-slate-800 tracking-tight">Cálculo de Fila de Produção Ótima</h2>
          <p className="text-slate-600 max-w-xl">
            Clique no botão ao lado para que o Motor AutoFlex Projedata analise o estoque disponível e calcule instantaneamente a sequência de produção que maximiza a rentabilidade da fábrica.
          </p>
        </div>
        <button
        ...
        <div className="bg-white border border-slate-200 shadow-xl shadow-slate-100 rounded-lg overflow-hidden mt-2">
          <div className="bg-[#003366] text-white px-6 py-4 flex justify-between items-center">
            <h3 className="font-extrabold uppercase tracking-wide text-sm flex items-center gap-2">
              ...
              Ordem de Produção Ativa e Sugerida
            </h3>
      ...
      {simulation && simulation.producedItems.length === 0 && (
        <div className="bg-yellow-50 border-l-4 border-yellow-500 text-yellow-900 px-6 py-6 rounded-lg text-center flex flex-col items-center justify-center gap-4">
          ...
          <div>
            <p className="font-extrabold text-lg">Incapacidade Produtiva Detectada</p>
            <p className="text-sm mt-1 max-w-lg">O motor AutoFlex Projedata rodou, mas concluiu que o estoque atual de insumos não é suficiente para fabricar nem mesmo uma unidade de qualquer produto. Revise o <span className="font-bold">Gerenciamento de Insumos</span> ou as receitas dos produtos.</p>
          </div>
        </div>
      )}
